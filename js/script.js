...
window.addEventListener('DOMContentLoaded', () => {
  ...
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '02-02-22');
...
function cards() {
  ...
  //         });
  //     });
  // fetch('https://food-js-e5db5-default-rtdb.europe-west1.firebasedatabase.app/menu.json')
  //     .then((response) => {
  ...
  //         console.log(data)
  //     })
  // Аналог тому, что написанно выше


  Object(_services_services__WEBPACK_IMPORTED_MODULE_0__["getResourse"])('https://food-js-e5db5-default-rtdb.europe-west1.firebasedatabase.app/menu.json').then(data => crateCart(data));

  function crateCart(data) {
    data.forEach(_ref => {
      let {
        img,
        altimg,
      ...
      element.innerHTML = `
                ...
                <div class="menu__item-price">
                    <div class="menu__item-cost">Цена:</div>
                    <div class="menu__item-total"><span>${price * 74}</span> грн/день</div>
                </div>
            `;
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      statusMessage.style.cssText = `
                ...
                margin: 0 auto;
            `;
      form.insertAdjacentElement('afterend', statusMessage); //request.setRequestHeader('Content-type', 'application/json; charset=utf-8');

      const formData = new FormData(form); //====
      // const object = {};
      // formData.forEach(function (value, key) {
      //     object[key] = value;
      // }); ====\\ равна с нижней записью

      const json = JSON.stringify(Object.fromEntries(formData.entries())); // fromEntries преобразует из массива в обьект
      //const json = JSON.stringify(formData.entries());
      // entries преобразует из обьекта в массив с массивами
      //Fetch
...
function openModal(modalSelector, modalTimerId) {
  ...
  document.body.style.overflow = 'hidden'; // или modal.classList.toggle('show');
...
function closeModal(modalSelector) {
  ...
  document.body.style.overflow = ''; // или modal.classList.toggle('show');
}

function modal(triggerSelector, modalSelector, modalTimerId) {
  // Modal
  const modalTrigger = document.querySelectorAll(triggerSelector),
        modal = document.querySelector(modalSelector); //появление модалки
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  const tabs = document.querySelectorAll(tabsSelector),
        ...
        tabsParent = document.querySelector(tabsParentSelector); // скрывает блоки
...
let getResourse = async url => {
  let res = await fetch(url); //Для выявления ошибок при запросе
